  ...
  // 🔧 CORECCIÓN: Siempre incluir arrays, aunque estén vacíos
  ...
  const result = tamanos
    .filter(t => {
      const isValid = t && t.nombre && t.nombre.trim() && t.precio !== undefined && t.precio >= 0;
      if (!isValid) {
  ...
  const result = extras
    .filter(e => {
      const isValid = e && e.nombre && e.nombre.trim() && e.precio !== undefined && e.precio >= 0;
      if (!isValid) {
